...
int main() {
    ...
    vvi to(n);
    rep(i,n){
        to[p[i]].push_back(q[i]);
        to[q[i]].push_back(p[i]);
        uf.merge(p[i],q[i]);
    }
    map<int,vi> grp;
    rep(i,n) grp[uf.find(i)].push_back(i);
    debug(grp);
    mint ans = 1;
    vc<mint> r(n+1);
    r[1] = 1;
    r[2] = 3;
    rep(i,3,n+1) r[i] = r[i-1] + r[i-2];
    for(auto[id,vc] : grp){
        int eg = 0;
        for(auto p:vc) eg += sz(to[p]);
        eg/=2; debug(eg);
        ans *= r[eg];
    }
    cout << ans << endl;
